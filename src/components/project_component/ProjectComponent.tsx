import React from 'react'
import '../../styles/project_component/ProjectComponent.css'
import { useParams } from 'react-router-dom';

const logoUrl = "data:image/png;base64,";

type ProjectComponentProps = {
    imgUrl: string,
    projectId: string,
    projectName: string
}


function ProjectComponent({ imgUrl, projectId, projectName }: ProjectComponentProps) {
    const params = useParams()
    return (
        <a className='project-component' href={`/${params.lang}/projects/details/` + projectId}>
            {imgUrl != '' ? <iframe src={imgUrl}></iframe> : <img src={imgUrl != '' ? imgUrl : logoUrl} />}
            <p>{projectName != '' ? projectName : "Sample Project"}</p>
        </a>
    )
}

export default ProjectComponent